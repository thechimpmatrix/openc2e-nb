...
static bool path_startswith(const fs::path& path, fs::path prefix) {
	// Remove trailing empty element (directory separator) from prefix
	if (prefix.has_filename() == false && prefix.has_parent_path()) {
	...
	for (auto pi = prefix_begin; pi != prefix_end; ++pi, ++path_begin) {
		if (path_begin == path_end || *pi != *path_begin) {
			return false;
		}
	}
	return true;
