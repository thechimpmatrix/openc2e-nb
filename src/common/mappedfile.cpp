...
mappedfile::mappedfile(const std::filesystem::path& filename) {
	...
	if (m_file == INVALID_HANDLE_VALUE) {
		NORN_ERR(FILEIO, fmt::format("Memory-mapped file open failed: {}, error = {}", filename.string(), GetLastError()));
	...
	if (GetFileSizeEx(m_file, &lifilesize) == 0) {
		...
		NORN_ERR(FILEIO, fmt::format("Memory-mapped file size query failed, error = {}", sizeerr));
		throw mappedfileerror(fmt::format("Error getting size of file, error = {}", sizeerr));
	};
	m_size = lifilesize.QuadPart;
	if (m_size == 0) {
		// TODO: allow empty?
		NORN_ERR(FILEIO, "Memory-mapped file failed: file is empty");
	...
	if (m_mapping == nullptr) {
		...
		CloseHandle(m_file); // TODO: handle error
		NORN_ERR(FILEIO, fmt::format("CreateFileMapping failed, error = {}", mappingerr));
	...
	if (m_view == nullptr) {
		...
		CloseHandle(m_file); // TODO: handle error
		NORN_ERR(FILEIO, fmt::format("MapViewOfFile failed, error = {}", viewerr));
	...
	if (fno == -1) {
		NORN_ERR(FILEIO, fmt::format("Memory-mapped file open failed: {}, errno = {}", filename.string(), errno));
	...
	if (m_size == static_cast<size_t>(-1)) {
		...
		NORN_ERR(FILEIO, fmt::format("Memory-mapped file seek failed, errno = {}", seekerr));
	...
	if (m_map == (void*)-1) {
		int maperr = errno;
		::close(fno); // TODO: handle error
		NORN_ERR(FILEIO, fmt::format("mmap failed, errno = {}", maperr));
