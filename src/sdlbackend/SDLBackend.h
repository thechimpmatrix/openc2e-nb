...
class SDLRenderTarget : public RenderTarget {
  public:
	...
	int32_t alloc_width = 0;  // dimensions used to create this texture (0 = main/screen)
	int32_t alloc_height = 0;
...
class SDLBackend : public Backend {
  ...
  protected:
	...
	std::vector<std::weak_ptr<SDLRenderTarget>> render_targets;  // tracked for D3D device reset
	float zoom_factor = 1.0f;  // >1.0 = zoomed in, <1.0 = zoomed out
