...
retry:
	...
	switch (event.type) {
		...
		case SDL_MOUSEMOTION:
			...
			e.x = static_cast<int>(event.motion.x / zoom_factor);
			e.y = static_cast<int>(event.motion.y / zoom_factor) - mainrendertarget->viewport_offset_top;
			e.xrel = static_cast<int>(event.motion.xrel / zoom_factor);
			e.yrel = static_cast<int>(event.motion.yrel / zoom_factor);
			e.button = 0;
		...
		case SDL_MOUSEBUTTONUP:
			...
			e.x = static_cast<int>(event.button.x / zoom_factor);
			e.y = static_cast<int>(event.button.y / zoom_factor) - mainrendertarget->viewport_offset_top;
			break;
		...
		case SDL_MOUSEWHEEL:
			...
			if (SDL_GetModState() & KMOD_CTRL) {
				if (event.wheel.y > 0) {
					zoom_factor = std::min(zoom_factor * 1.1f, 4.0f);
				} else if (event.wheel.y < 0) {
					zoom_factor = std::max(zoom_factor / 1.1f, 0.5f);
				}
				printf("* Zoom: %.1fx\n", zoom_factor);
				goto retry;
			}
...
int32_t SDLRenderTarget::getWidth() const {
	...
	if (texture) {
		...
		width = static_cast<int>(width / parent->zoom_factor);
	}
...
int32_t SDLRenderTarget::getHeight() const {
	...
	if (texture) {
		...
		height = static_cast<int>(height / parent->zoom_factor);
	}
...
std::shared_ptr<RenderTarget> SDLBackend::newRenderTarget(int32_t w, int32_t h) {
	...
	newtarget->alloc_width = w;
	newtarget->alloc_height = h;

	// Track for recreation after D3D device reset
	render_targets.push_back(newtarget);

	return std::dynamic_pointer_cast<RenderTarget>(newtarget);
}

void SDLBackend::recreateRenderTargets() {
	printf("* SDL render targets reset — recreating textures\n");

	// Remove expired weak_ptrs and recreate live ones
	auto it = render_targets.begin();
	while (it != render_targets.end()) {
		if (auto target = it->lock()) {
			if (target->alloc_width > 0 && target->alloc_height > 0) {
				if (target->texture) {
					SDL_DestroyTexture(target->texture);
				}
				target->texture = SDL_CreateTexture(renderer, 0, SDL_TEXTUREACCESS_TARGET,
					target->alloc_width, target->alloc_height);
				if (target->texture) {
					printf("* SDL recreated rendertarget texture %p (%dx%d)\n",
						target->texture, target->alloc_width, target->alloc_height);
				} else {
					printf("* SDL FAILED to recreate rendertarget: %s\n", SDL_GetError());
				}
			}
			++it;
		} else {
			it = render_targets.erase(it);
		}
	}
}
	...
	while (true) {
		...
		if (windowwidth > 0 && windowheight > 0) {
			SDL_RenderSetScale(renderer,
				drawablewidth * zoom_factor / windowwidth,
				drawableheight * zoom_factor / windowheight);
		}
