...
Sound SoundManager::playSound(std::string name, bool loop) {
	...
	std::string filename = findSoundFile(name + ".wav").string();
	if (filename.size() == 0) {
		NORN_WRN(AUDIO, fmt::format("Sound file not found: '{}.wav'", name));
		if (engine.version < 3)
	...
	if (!handle) {
		NORN_ERR(AUDIO, fmt::format("Sound playback failed: {}", filename));
...
Sound SoundManager::playVoice(std::string name) {
	...
	std::string filename = findSoundFile(name + ".wav").string();
	if (filename.size() == 0) {
		NORN_WRN(AUDIO, fmt::format("Voice file not found: '{}.wav'", name));
		if (engine.version < 3)
			return {}; // creatures 1 and 2 ignore non-existent audio clips
		throw Exception(fmt::format("No such clip '{}.wav'", name));
	}
	...
	if (!handle) {
		NORN_ERR(AUDIO, fmt::format("Voice playback failed: {}", filename));
		// note that more specific error messages can be thrown by implementations of play_clip
		throw Exception("failed to load audio clip " + filename);
	}
