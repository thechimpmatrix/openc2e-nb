...
void prayManager::addFile(const fs::path& filename) {
	...
	NORN_INF(FILEIO, fmt::format("PRAY file loaded: {} ({} blocks)", filename.string(), metadata.size()));
}
...
void prayManager::update() {
	...
	if (!catalogue.hasTag("Pray System File Extensions")) {
		NORN_WRN(FILEIO, "Catalogue tag \"Pray System File Extensions\" not found; no PRAY files will be loaded");
	...
	for (auto ext : extensions) {
		for (auto f : findAgentFiles("*." + ext)) {
			...
			try {
				...
				NORN_ERR(FILEIO, fmt::format("PRAY file parse error in \"{}\": {}", f.string(), e.what()));
