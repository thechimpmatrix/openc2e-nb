...
void c2eCreature::handleStimulus(c2eStim& stim) {
	...
	// Deliver stimulus event to PythonBrain so it can drive attention
	if (auto* pb = dynamic_cast<PythonBrain*>(brain.get())) {
		pb->pushStimulus(stim.noun_id, stim.verb_id, stim.noun_amount);
	}
}
