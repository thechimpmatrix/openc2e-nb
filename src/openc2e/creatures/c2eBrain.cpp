...
void c2eTract::setupTract() {
	...
	c2eLobe* srclobe = b->lobes[srclobename].get();
	c2eLobe* destlobe = b->lobes[destlobename].get();
...
void c2eBrain::processGenes() {
	for (auto& i : parent->getGenome()->genes) {
		...
		if (typeid(*g) == typeid(c2eBrainLobeGene)) {
			auto l = std::make_unique<c2eLobe>(this, (c2eBrainLobeGene*)g);
			components.insert(l.get());
			lobes[l->getId()] = std::move(l);
		} else if (typeid(*g) == typeid(c2eBrainTractGene)) {
			auto t = std::make_unique<c2eTract>(this, (c2eBrainTractGene*)g);
			components.insert(t.get());
			tracts.push_back(std::move(t));
		}
	}
}
