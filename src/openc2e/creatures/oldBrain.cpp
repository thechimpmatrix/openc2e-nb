...
void oldLobe::tick() {
	...
	for (unsigned int i = 0; i < neurons.size(); i++) {
		unsigned char out = processSVRule(&neurons[i], nullptr, staterule);
	...
	if (ourGene->flags & 1) {
		...
		unsigned char* bestoutput = nullptr;
		for (auto& neuron : neurons) {
...
unsigned int oldNeuron::magicalHash(unsigned int type) {
	...
	for (unsigned int t = type; t <= type_limit; t++) {
		for (auto& d : dendrites[t]) {
			...
			unsigned int v = reinterpret_cast<uintptr_t>(d.src);
			outsum += v;
			out *= v;
		}
	}
...
void oldBrain::processGenes() {
	...
	for (auto& gene : genome->genes) {
		...
		if (oldBrainLobeGene* g = dynamic_cast<oldBrainLobeGene*>(gene.get())) {
			lobes.push_back(std::make_unique<oldLobe>(this, g));
		}
	}
