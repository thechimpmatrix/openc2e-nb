...
Agent::Agent(unsigned char family_, unsigned char genus_, unsigned short species_, unsigned int zorder_)
	: timerrate(0), vm(nullptr), attr(0), visible(true) {
	initialized = false;
	lifecount = 0;
...
void Agent::finishInit() {
	...
	initialized = true;
	NORN_DBG(AGENT, fmt::format("Agent created: family={} genus={} species={} zorder={}", (int)family, (int)genus, (int)species, zorder));
}
...
void Agent::unhandledException(std::string info, bool wasscript) {
	NORN_ERR(AGENT, fmt::format("Unhandled exception in agent family={} genus={} species={} script={}: {}", (int)family, (int)genus, (int)species, lastScript, info));
...
void Agent::vmTick() {
	...
	while (vm && vm->timeslice && !vm->isBlocking() && !vm->stopped()) {
		...
		try {
			...
			NORN_WRN(AGENT, fmt::format("Invalid agent exception in {} script {}: {}", identify(), lastScript, e.prettyPrint()));
			...
			NORN_ERR(AGENT, fmt::format("Exception in {} script {}: {}", identify(), lastScript, e.prettyPrint()));
			unhandledException(e.prettyPrint(), true);
		}
...
Agent::~Agent() {
	assert(lifecount == 0);
	NORN_DBG(AGENT, fmt::format("Agent destroyed: family={} genus={} species={}", (int)family, (int)genus, (int)species));
...
void Agent::kill() {
	...
	if (vm) { vm->stop(); world.freeVM(vm); vm = nullptr; }
	if (sound) { sound.stop(); sound = {}; }

	world.queueDestroy(shared_from_this());
}
...
bool Agent::beDropped() {
	...
	carriedby = AgentRef(nullptr);
	bool wasinvehicle = invehicle;
	invehicle = AgentRef(nullptr);
