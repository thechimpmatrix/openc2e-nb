...
World::World() {
	...
	gallery = std::make_unique<imageManager>();
	history = std::make_unique<historyManager>();
	map = std::make_unique<Map>();
	praymanager = std::make_unique<prayManager>();
	scriptorium = std::make_unique<Scriptorium>();
}
...
void World::init() {
	NORN_INF(WORLD, "World::init() starting");
	...
	NORN_INF(WORLD, "World::init() complete");
}
...
void World::queueDestroy(std::shared_ptr<Agent> agent) {
	if (agent) pending_destroy_.push_back(std::move(agent));
}

void World::flushPendingDestroys() {
	if (pending_destroy_.empty()) return;
	NORN_DBG(WORLD, fmt::format("NB: flushing {} pending destroys", pending_destroy_.size()));
	for (auto& agent : pending_destroy_) {
		if (agent) agent->finalDestroy();
	}
...
void World::tick() {
	...
	while (i != agents.end()) {
		...
		i++;
		if (a->isPendingKill()) continue;  // NB: skip agents awaiting end-of-frame destruction
		a->tick();
	}
	...
	// NB: flush deferred destructions at end of tick
	...
	for (auto a : world.agents) {
		...
		for (std::map<unsigned int, std::shared_ptr<OutputPort> >::iterator p = a->outports.begin();
			...
			for (PortConnectionList::iterator c = p->second->dests.begin(); c != p->second->dests.end(); c++) {
				if (!c->first || c->first->isPendingKill())
					continue;
...
void World::executeInitScript(std::string x) {
	...
	NORN_DBG(WORLD, fmt::format("Executing bootstrap script: {}", x));
	try {
		...
		NORN_ERR(WORLD, fmt::format("Bootstrap script \"{}\" failed: {}", fs::path(x).filename().string(), e.prettyPrint()));
...
void World::executeBootstrap(bool switcher) {
	if (engine.version < 3) {
		...
		std::string edenpath = findMainDirectoryFile("Eden.sfc").string();
	...
	NORN_INF(WORLD, fmt::format("executeBootstrap called (switcher={})", switcher));
	if (switcher) {
		executeBootstrap(getWorldSwitcherBootstrapDirectory().string());
	} else {
		if (engine.version == 3) {
