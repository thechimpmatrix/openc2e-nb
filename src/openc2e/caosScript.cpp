...
caosScript::caosScript(const std::string& dialect, const std::string& _filename) {
	...
	if (!d) {
		NORN_ERR(CAOS, fmt::format("Unknown CAOS dialect '{}' in {}", dialect, _filename));
		throw parseException(std::string("Unknown dialect ") + dialect);
	}
	current = installer = std::make_shared<script>(d, _filename);
	filename = _filename;
}
...
void caosScript::parse(const std::string& caostext) {
	...
	try {
		parseloop(ST_INSTALLER, nullptr);

		std::string buf;
		auto tokinfo = std::make_shared<std::vector<toktrace>>();
		...
		NORN_ERR(CAOS, fmt::format("CAOS compilation error in '{}': {}", filename, e.what()));
		e.filename = filename;
		if (!tokens)
			throw;
		...
		e.context = std::make_shared<std::vector<caostoken>>();
...
const cmdinfo* caosScript::readCommand(caostoken* t, const std::string& prefix, bool except) {
	...
	if (!ci) {
		if (!except)
			return nullptr;
		NORN_WRN(CAOS, fmt::format("Command not found: '{}' in {}", fullname, filename));
		throw parseException(std::string("Command not found: ") + fullname);
	}

	// See if there's a subcommand
	caostoken* t2 = nullptr;
	const cmdinfo* subci = nullptr;
...
std::shared_ptr<CAOSExpression> caosScript::readExpr(const enum ci_type xtype) {
	...
	if (xtype == CI_BAREWORD) {
		if (logicalType(t) == TOK_WORD) {
			return std::make_shared<CAOSExpression>(errindex, caosValue(t->word()));
		} else if (logicalType(t) == TOK_CONST) {
			if (asConst(*t).getType() != CAOSSTR)
				unexpectedToken(*t);
			return std::make_shared<CAOSExpression>(errindex, asConst(*t));
		} else {
			unexpectedToken(*t);
		}
		assert(!"UNREACHABLE");
	...
	switch (logicalType(t)) {
		case TOK_CONST:
			return std::make_shared<CAOSExpression>(errindex, asConst(*t));
		case TOK_BYTESTR:
			return std::make_shared<CAOSExpression>(errindex, t->bytestr());
		case TOK_WORD: break; // fall through to remainder of function
		default: unexpectedToken(*t);
	}

	std::string oldpayload = t->word();
	auto ce = std::make_shared<CAOSExpression>(errindex, CAOSCmd());
	...
	if (t->word().size() == 4 && isdigit(t->word()[2]) && isdigit(t->word()[3])) {
		if (!strncmp(t->word().c_str(), "va", 2) || !strncmp(t->word().c_str(), "ov", 2) || !strncmp(t->word().c_str(), "mv", 2)) {
			...
			auto arg = std::make_shared<CAOSExpression>(errindex, caosValue(idx));
			cmd->op = op;
...
void caosScript::parseloop(int state, void* info) {
	...
	while ((t = getToken(ANYTOKEN))) {
		...
		if (t->word() == "scrp") {
			...
			int spcs = bits[2];
			int scrp = bits[3];
			scripts.push_back(std::make_shared<script>(d, filename, fmly, gnus, spcs, scrp));
			...
			if (!removal)
				removal = std::make_shared<script>(d, filename);
			current = removal;
		} else if (t->word() == "iscr") {
			if (state == ST_INSTALLER || state == ST_BODY || state == ST_REMOVAL)
				state = ST_INSTALLER;
...
void CAOSExpression::eval(caosScript* scr, bool save_here) const {
	if (auto* cmd_p = value.get_if<CAOSCmd>()) {
		...
		for (size_t i = cmd.arguments.size(); i-- > 0; ) {
