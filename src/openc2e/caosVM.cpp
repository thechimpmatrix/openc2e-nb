...
void caosVM::startBlocking(std::unique_ptr<blockCond> whileWhat) {
	if (!owner)
...
inline void caosVM::runOpCore(script* s, caosOp op) {
	switch (op.opcode) {
		...
		default:
			NORN_ERR(CAOS, fmt::format("Illegal opcode {} in {}", (int)op.opcode, currentscript ? currentscript->filename : "<unknown>"));
...
inline void caosVM::runOp() {
	...
	runops++;
	if (runops > 1000000) {
		NORN_ERR(CAOS, fmt::format("Script exceeded 1m ops in {}", currentscript ? currentscript->filename : "<unknown>"));
	...
	try {
		...
		NORN_ERR(CAOS, fmt::format("CAOS exception in {}: {}", currentscript ? currentscript->filename : "<unknown>", e.what()));
		e.trace(currentscript, op.traceindex);
		stop();
		throw;
	} catch (Exception& e) {
		NORN_ERR(CAOS, fmt::format("Exception in {}: {}: {}", currentscript ? currentscript->filename : "<unknown>", demangle(typeid(e).name()), e.what()));
...
void caosVM::runEntirely(std::shared_ptr<script> s) {
	...
	while (true) {
		...
		if (blocking) {
			blocking.reset();
			NORN_WRN(CAOS, fmt::format("Blocking in installation script {}", s ? s->filename : "<unknown>"));
