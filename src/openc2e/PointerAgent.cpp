...
void PointerAgent::tick() {
	if (!paused) {
		...
		if (agent_under_pointer) {
			auto locked = agent_under_pointer.safeGet();  // NB: safe access — may be pending_kill
			if (locked) {
				oldpart = locked->part(part_under_pointer);
			} else {
		...
		if (a) {
			...
			parent = a->getParent();
			// NB: reject pending_kill parents — treat as no agent
	...
	if (event.type == eventmousemove) {
		...
		if (event.button == buttonleft) {
			...
			if (a) {
				...
				if (eve != -1) {
					Agent* p = a ? a->getParent() : nullptr;
					if (p && !p->isPendingKill()) {  // NB: guard against pending_kill
