...
MetaRoom* Map::getFallbackMetaroom() {
	return metarooms.size() == 0 ? nullptr : metarooms[0].get();
}
...
MetaRoom* Map::addMetaRoom(int _x, int _y, int _width, int _height, const std::string& back, bool wrap) {
	...
	NORN_INF(WORLD, fmt::format("MetaRoom created: id={} pos=({},{}) size={}x{} wrap={}", metarooms.back()->id, _x, _y, _width, _height, wrap));
...
int Map::getDoorPerm(const std::shared_ptr<Room>& r1, const std::shared_ptr<Room>& r2) {
	if (!hasDoor(r1, r2)) {
		NORN_ERR(WORLD, fmt::format("Room lookup failure: door between rooms {} and {} does not exist", r1->id, r2->id));
