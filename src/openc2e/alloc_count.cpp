...
#include <windows.h>
#define atomic_increment(v) InterlockedIncrement64(&(v))
#define atomic_decrement(v) InterlockedDecrement64(&(v))
#define set_if_eq(v, checkval, newval) (InterlockedCompareExchange64(&(v), (newval), (checkval)) == (checkval))
...
void AllocationCounter::increment() {
	int64_t newCount = atomic_increment(curCount);
