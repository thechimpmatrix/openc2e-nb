...
void c_APPR(caosVM* vm) {
	...
	vm->startBlocking(std::make_unique<blockUntilApproached>(vm->targ, vm->_it_));
}
...
void c_NEWC(caosVM* vm) {
	...
	try {
		...
		NORN_ERR(CREATURE, fmt::format("Silently caught exception in NEWC (SkeletalCreature creation): family={} sex={} variant={}", family, sex, variant));
		delete a;
		throw;
	}
...
void c_NEW_CREA_c1(caosVM* vm) {
	...
	try {
		...
		NORN_ERR(CREATURE, fmt::format("Silently caught exception in NEW:CREA (c1/c2 creature creation): sex={} engine_version={}", sex, engine.version));
		delete a;
		throw;
	}
...
void c_NEW_CRAG(caosVM* vm) {
	...
	try {
		...
		NORN_ERR(CREATURE, fmt::format("Silently caught exception in NEW:CRAG (CompoundCreature creation): family={} sex={} variant={}", family, sex, variant));
		delete a;
		throw;
	}
