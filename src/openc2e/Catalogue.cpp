...
void Catalogue::addFile(fs::path path) {
	...
	try {
		...
		NORN_INF(FILEIO, fmt::format("Catalogue file loaded: {}", path.string()));
	} catch (const catalogueException& ex) {
		NORN_ERR(FILEIO, fmt::format("Catalogue parse error in {}: {}", path.string(), ex.what()));
...
void Catalogue::initFrom(fs::path path, std::string language) {
	...
	for (fs::directory_iterator i(path); i != end; ++i) {
		try {
			...
			NORN_ERR(FILEIO, fmt::format("Catalogue directory iteration error on '{}': {}", i->path().filename().string(), ex.what()));
