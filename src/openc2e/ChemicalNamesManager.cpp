...
const std::vector<std::string>& ChemicalNamesManager::getChemicalNames() {
	if (!chemical_names_loaded) {
		if (engine.version < 3) {
			...
			chemical_names = ReadStrFile(findMainDirectoryFile("allchemicals.str").string());
		} else {
...
const std::vector<std::pair<std::string, std::vector<int>>>& ChemicalNamesManager::getChemicalGroups() {
	if (!chemical_groups_loaded) {
		...
		if (engine.version == 2) {
			...
			auto lines = ReadVerticalBarSeparatedValuesFile(findMainDirectoryFile("Applet Data/ChemGroups").string());
