...
static fs::path findFile(FileDirectory type, fs::path name) {
	...
	for (auto d : data_directories) {
		...
		if (!err) {
			NORN_DBG(FILEIO, fmt::format("File resolved: {} -> {}", name.string(), resolved.string()));
			return resolved;
		}
	}
...
static std::vector<fs::path> findByWildcard(FileDirectory type, std::string wild) {
	...
	for (auto d : data_directories) {
		...
		for (const auto& entry : case_insensitive_filesystem::directory_iterator(dirname)) {
			if (wildcard_match_ignore_case(wild, entry.lexically_relative(dirname).string())) {
...
std::vector<fs::path> findCobFiles(std::string wild) {
	if (engine.version == 2) {
		return findByWildcard(DIRECTORY_MAIN, (fs::path("Objects") / wild).string());
	} else {
...
fs::path homeDirectory() {
	...
	if (SUCCEEDED(hr)) {
		...
		throw Exception("SHGetKnownFolderPath failed to locate the Documents folder");
	}
	if (!fs::exists(p) || !fs::is_directory(p))
		throw Exception("Windows reported that your Documents folder is at '" + p.string() + "' but there's no directory there!");
#endif
