...
void CobManager::update() {
	...
	for (auto cob : findCobFiles("*.cob")) {
		if (engine.version == 1) {
			c1cobfile cobfile = read_c1cobfile(cob.string());
			objects.emplace_back(cobfile.name, cob.string());
			auto rcb = cob;
			...
			c2cobfile cobfile(cob.string());

			for (auto b : cobfile.blocks) {
				if (b->type != "agnt")
					continue;
				cobAgentBlock a(b);
				objects.emplace_back(a.name, cob.string());
...
void CobManager::remove(const CobFileInfo& info) {
	std::string rdata;
	if (engine.version == 1) {
		std::string rcbpath = findCobFile(fs::path(info.filename).stem().string() + ".rcb").string();
