...
// Track which missing files we've already warned about (warn once per file)
static std::set<std::string> s_missing_image_warned;
...
std::shared_ptr<creaturesImage> tryOpenImage(std::string fname) {
	...
	// Missing sprites are expected for C3-only installs (DS-only files like hand.c16).
	// Log once at INF level and return null — callers already handle null gracefully.
	if (s_missing_image_warned.insert(fname).second) {
		NORN_INF(RENDER, fmt::format("Missing sprite '{}' (DS-only file?)", fname));
	}
	return {};
}
...
std::shared_ptr<creaturesImage> tryOpenBackground(std::string fname) {
	...
	if (exists(realfile)) {
		auto img = std::make_shared<creaturesImage>(basename);
		img->images = ImageUtils::ReadImage(realfile.string());
		NORN_DBG(RENDER, fmt::format("Background loaded: {}", realfile.string()));
		return img;
	}
	NORN_WRN(RENDER, fmt::format("Background file not found: {}", fname));
	return {};
}
...
void imageManager::loadDefaultPalette() {
	if (engine.gametype == "c1") {
		...
		if (exists(palpath) && !is_directory(palpath)) {
			default_palette = ReadPaletteFile(palpath.string());
		} else {
...
std::shared_ptr<creaturesImage> imageManager::getBackground(const std::string& name, unsigned int metaroom_width, unsigned int metaroom_height) {
	...
	if (engine.version == 1) {
		...
		NORN_ERR(RENDER, "Don't know how to load backgrounds for current engine type");
		throw Exception("Don't know how to load backgrounds for current engine type");
	}
	if (!img) {
		NORN_WRN(RENDER, fmt::format("Background not found: '{}'", name));
	...
	if (engine.version == 1) {
		...
		if (img) {
			...
			if (!hedfilename.empty()) {
				hedfile hed = read_hedfile(hedfilename.string());
		...
		NORN_ERR(RENDER, "Don't know how to load sprites for current engine type");
		throw Exception("Don't know how to load sprites for current engine type");
	}

	if (img) {
		images[name] = img;
		NORN_DBG(RENDER, fmt::format("Sprite loaded into gallery: '{}'", name));
	} else {
		NORN_WRN(RENDER, fmt::format("Sprite not found: '{}'", name));
...
std::shared_ptr<creaturesImage> imageManager::getCharsetDta(imageformat format,
	...
	std::string filename = findImageFile("EuroCharset.dta").string();
	if (filename.empty()) {
		filename = findImageFile("CHARSET.DTA").string();
	}
	if (filename.empty()) {
		return {};
	}
	...
	auto img = std::make_shared<creaturesImage>(path(filename).stem().string());
	img->images = images;
	return img;
}
