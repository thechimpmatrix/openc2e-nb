...
std::optional<PrayTagBlock> maybe_get_block_as_tags(PrayFileReader& file, int i) {
	...
	try {
		auto [int_tags, string_tags] = file.getBlockTags(i);
		for (const auto& [key, value] : int_tags) {
			if (!is_printable(key)) {
				return {};
			}
		}
		for (const auto& [key, value] : string_tags) {
			if (!is_printable(key) || !is_printable(value)) {
				return {};
			}
		}
		fmt::print(stderr, "Successfully parsed unknown block type '{}' as a tag block\n", file.getBlockType(i));
		return PrayTagBlock{int_tags, string_tags};
	} catch (...) {
		return {};
	}
}
...
int main(int argc, char** argv) {
	...
	for (size_t i = 0; i < file.getNumBlocks(); i++) {
		...
		if (tags) {
			...
			auto& [int_tags, string_tags] = *tags;
