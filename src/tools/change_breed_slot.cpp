...
std::string get_new_filename(std::string filename, int new_species_number, int new_slot_number) {
	...
	newname = (fs::path(filename).parent_path() / newname).string();
	return newname;
}
	...
	for (size_t i = 0; i < reader.getNumBlocks(); ++i) {
		if (reader.getBlockType(i) == "EGGS") {
			...
			for (auto& kv : tags.second) {
				std::string value_extension = fs::path(kv.second).extension().string();
			...
			std::string block_extension = fs::path(blockname).extension().string();
